...
label0:
    ...
    {
        s = s.replace("%2A", "*").replace("%2a", "*").replace("%20", " ");
        try
        {
    ...
    public static List decodeParameters(String s)
    {
        ...
        for(int i = 0; i < j; i++)
        {
            ...
            if(as.length != 2)
                continue;
            String s1 = decode(as[0]);
            String s2 = decode(as[1]);
            if(!s1.equals("") && !s2.equals(""))
                arraylist.add(new HttpParameter(s1, s2));
        }
    ...
    {
        return compareTo((HttpParameter)obj);
    }

    public int compareTo(HttpParameter httpparameter)
    {
        int j = name.compareTo(httpparameter.name);
        int i = j;
        if(j == 0)
    ...
    private static final long serialVersionUID = 0x88625a4L;
    private File file;
