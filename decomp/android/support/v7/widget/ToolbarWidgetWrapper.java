    ...
    implements DecorToolbar
{
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean flag, int i, int j)
    {
        ...
        mDefaultNavigationIcon = toolbar.getDrawable(android.support.v7.appcompat.R.styleable.ActionBar_homeAsUpIndicator);
        if(flag)
        {
            ...
            j = toolbar.getResourceId(android.support.v7.appcompat.R.styleable.ActionBar_customNavigationLayout, 0);
            if(j != 0)
            {
                setCustomView(LayoutInflater.from(mToolbar.getContext()).inflate(j, mToolbar, false));
                setDisplayOptions(mDisplayOpts | 0x10);
            }
            j = toolbar.getLayoutDimension(android.support.v7.appcompat.R.styleable.ActionBar_height, 0);
            if(j > 0)
            {
            ...
            j = toolbar.getDimensionPixelOffset(android.support.v7.appcompat.R.styleable.ActionBar_contentInsetStart, -1);
            int k = toolbar.getDimensionPixelOffset(android.support.v7.appcompat.R.styleable.ActionBar_contentInsetEnd, -1);
            if(j >= 0 || k >= 0)
                mToolbar.setContentInsetsRelative(Math.max(j, 0), Math.max(k, 0));
            j = toolbar.getResourceId(android.support.v7.appcompat.R.styleable.ActionBar_titleTextStyle, 0);
            if(j != 0)
                mToolbar.setTitleTextAppearance(mToolbar.getContext(), j);
            j = toolbar.getResourceId(android.support.v7.appcompat.R.styleable.ActionBar_subtitleTextStyle, 0);
            if(j != 0)
                mToolbar.setSubtitleTextAppearance(mToolbar.getContext(), j);
            j = toolbar.getResourceId(android.support.v7.appcompat.R.styleable.ActionBar_popupTheme, 0);
            if(j != 0)
